    ...
    public class WitClientUtils
    {
        ...
        public bool AddAndSaveLink(WiLink link, WorkItem wi)
        {
            ...
            if (parsedLink != null)
            {
                try
                {
                    var targetWorkItem = GetWorkItem(link.TargetWiId);
        ...
        public bool RemoveAndSaveLink(WiLink link, WorkItem wi)
        {
            ...
            var linkToRemove = wi.Relations.OfType<WorkItemRelation>().SingleOrDefault(
                rl =>
                    rl.Rel == link.WiType
        ...
        public bool ApplyAttachments(WiRevision rev, WorkItem wi, Dictionary<string, WiAttachment> attachmentMap, IsAttachmentMigratedDelegate<string, string, bool> isAttachmentMigratedDelegate)
        {
            ...
            foreach (var att in rev.Attachments)
            {
                try
                {
                    ...
                    {
                        var existingAttachment = IdentifyAttachment(att, wi, isAttachmentMigratedDelegate);
        ...
        private void AddRemoveAttachment(WorkItem wi, string filePath, string comment, AttachmentOperation op)
        {
            ...
            {
                var attachmentRelation = new WorkItemRelation();
                ...
                var attachmentRelation = wi.Relations.FirstOrDefault(e => e.Rel == "AttachedFile" && e.Attributes["filePath"].ToString() == filePath);
        ...
        public bool CorrectDescription(WorkItem wi, WiItem wiItem, WiRevision rev, IsAttachmentMigratedDelegate<string, string, bool> isAttachmentMigratedDelegate)
        {
            ...
            var description = wi.Fields[WiFieldReference.WorkItemType].ToString() == "Bug" ? wi.Fields[WiFieldReference.ReproSteps].ToString() : wi.Fields[WiFieldReference.Description].ToString();
        ...
        public void CorrectComment(WorkItem wi, WiItem wiItem, WiRevision rev, IsAttachmentMigratedDelegate<string, string, bool> isAttachmentMigratedDelegate)
        {
            ...
            var currentComment = wi.Fields[WiFieldReference.History].ToString();
        ...
        {
            // Save attachments
            foreach (var attachment in rev.Attachments)
            {
        ...
        private void SaveWorkItemFields(WorkItem wi)
        {
            ...
            foreach (var key in wi.Fields.Keys)
            {
                if (new string[] { 
        ...
        private void CorrectImagePath(WorkItem wi, WiItem wiItem, WiRevision rev, ref string textField, ref bool isUpdated, IsAttachmentMigratedDelegate<string, string, bool> isAttachmentMigratedDelegate)
        {
            ...
            foreach (var att in wiItem.Revisions.SelectMany(r => r.Attachments.Where(a => a.Change == ReferenceChangeType.Added)))
            {
                ...
                if (textField.Contains(fileName) || textField.IndexOf(encodedFileName, StringComparison.OrdinalIgnoreCase) >= 0 || textField.Contains("_thumb_" + att.AttOriginId))
                {
                    ...
                    if (tfsAtt != null)
                    {
                        var imageSrcPattern = $"src.*?=.*?\"([^\"])(?=.*{att.AttOriginId}).*?\"";
        ...
        {
            // Upload attachment
            var attachment = _witClientWrapper.CreateAttachment(att);
            ...
            var attachmentPatchDocument = new JsonPatchDocument
            {
                new JsonPatchOperation
                {
                    Operation = Operation.Add,
        ...
        {
            var existingAttachmentRelation =
        ...
        {
            var rel = sourceWI.Relations.SingleOrDefault(a =>
                a.Rel == link.WiType
            ...
            var relIndex = sourceWI.Relations.IndexOf(rel);
        ...
        {
            var linkTypes = _witClientWrapper.GetRelationTypes();
            var linkType = linkTypes.SingleOrDefault(lt => lt.ReferenceName == link.WiType);
        ...
        {
            if (isAttachmentMigratedDelegate(att.AttOriginId, out var attWiId))
            {
        ...
        private WorkItemRelation ResolveCyclicalLinks(WorkItemRelation link, WorkItem wi)
        {
            ...
            {
                var reverseLink = new WorkItemRelation();
                var reverseLinkTypeName = GetReverseLinkTypeReferenceName(link.Rel);
